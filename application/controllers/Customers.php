...
class Customers extends CI_Controller {
...

    {
        if($this->input->post('is_search') == 1) {
            ...
            $c = $this->members->search_orders($keyword);

        } else {

            $c = $this->members->get_all_orders();
        }
    ...
    public function upload(){
        ...
        if($_POST) {
            ...
            $newname = $oid . "-" .$_FILES["output"]['name'];
            $this->Member_model->update_output($oid, $newname);
            $this->uploadFile($newname);

            $data['message'] = "File has been uploaded";
        } 

        $data['output_file'] = $this->Member_model->get_output_file($this->input->get('oid'));

        $this->load->view('customers/upload',$data);
